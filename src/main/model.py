...
class IndividualEncoder(nn.Module):  # obtain the initial representations of each user (before learning context graph)
...
class WeightEncoder(nn.Module):  # to obtain the weight information
    def __init__(self):
        super().__init__()
...
class Model(nn.Module):
    def __init__(self, numerical_dim, categorical_dim, description_dim, tweet_dim, node_dim, act_fn, dropout,
        ...
        self.weight_encoder = WeightEncoder()  # weight information
        self.topological_encoder = nn.Linear(node_dim, node_dim)  # topology information
    ...
    def forward(self, data, return_graph=False, return_out=False):
        ...
        individual_reps_head = individual_reps.unsqueeze(-1) * self.similarity_head_w  # multi-head similarity
        ...
        for index, item in enumerate(data['context']):
            ...
            # the learned dynamic threshold
            weight = self.similarity_weight * initial_weight + (1 - self.similarity_weight) * offset_weight

            topological = individual_reps[user]
            topological = topological[weight > 0]  # sever as eq. 11 in the paper
            weight_reps.append(self.weight_encoder(weight[weight > 0]))  # weight information
            ...
            if len(topological) == 0:
                ...
                topological = self.topological_encoder(topological).mean(0)  # topology information

                # sample positive and negative samples to calculate homophily loss of the hybrid loss
        ...
        if return_graph:  # return the learned context graph or not
        ...
        reps = torch.cat([individual_reps[:data['batch_size']],
                          ...
                          weight_reps], dim=-1)  # the representations, refer to eq. 15 in the paper
        if return_out:  # return the representations for visualization
            out = list(self.cls)[0](reps)
            out = list(self.cls)[1](out)
            return out
        cls = self.cls(reps)
        cls_loss = self.cls_loss(cls, data['label'][:data['batch_size']])  # the classification loss

        # calculate the homophily loss, refer to eqs. 17-19 in the paper
