...
# To overcome the possible unbalance distribution, we employ a very simple augmentation.
# Namely, repeat the sample with fewer label count
...
# make sure accessing the whole preprocessed data
class MyDataset(Dataset):
    def __init__(self, dataset_name, top_k_count, num_hops, threshold):
        ...
        social_context = json.load(open('../context_graph/contexts/{}_{}_hops_social_context.json'.
                                        format(dataset_name, num_hops)))  # load the candidate
        user_without_description = set(json.load(
                open('../../dataset/{}/{}_user_without_description.json'.format(dataset_name, dataset_name)))
            )  # removing users without description (avoiding noise)
        self.social_context = []

        # obtain the initial context graph
        src, tgt = [], []
